...
/**
 * Script para migrar publicaciones existentes de posts.json a archivos individuales
 * Solo ejecutar una vez para migrar las publicaciones existentes
 */
...
function main() {
    console.log('🔄 Migrando publicaciones existentes...\n');
    
    if (!fs.existsSync(POSTS_JSON)) {
        console.log('❌ No se encontró posts.json');
        return;
    }
    
    const posts = JSON.parse(fs.readFileSync(POSTS_JSON, 'utf8'));
    console.log(`📚 Encontradas ${posts.length} publicaciones en posts.json\n`);
    ...
    posts.forEach((post, index) => {
        ...
        // Crear objeto sin id y featured (se manejará automáticamente)
        const publicacion = {
